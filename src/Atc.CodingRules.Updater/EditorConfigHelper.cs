...
public static class EditorConfigHelper
{
        ...
        IList<Tuple<string, List<string>>> suppressionLinesPrAnalyzer)
    {
        ...
        // Formatted with an explicit culture: without one this picked up the machine's locale, so
        // the same command run by two developers produced a different committed .editorconfig.
        lines.Add($"# generated @ {DateTime.Now.ToString("F", GlobalizationConstants.EnglishCultureInfo)}");
        lines.Add($"# {CodingRulesUpdaterVersionHelper.GetGeneratedByText()}");
