...
namespace Atc.CodingRules.Updater.CLI
{
    public static class EditorConfigHelper
    {
            ...
            DirectoryInfo rootPath)
        {
            ...
            var hasSection = lines.Any(x => x.StartsWith(AutogeneratedCustomSectionHeaderPrefix, StringComparison.Ordinal));
            if (!hasSection)
            {
            ...
            IList<Tuple<string, List<string>>> suppressionLinesPrAnalyzer)
        {
            ...
            lines.Add($"{AutogeneratedCustomSectionHeaderPrefix} {DateTime.Now:F}");
            lines.Add(SectionDivider);
        ...
        {
            var result = lines
                .TakeWhile(line => !line.StartsWith(AutogeneratedCustomSectionHeaderPrefix, StringComparison.Ordinal))
                .ToList();
        ...
        private static string[] ExtractAfterCustomAutogeneratedRulesContent(IEnumerable<string> lines)
        {
            ...
            foreach (var line in lines)
            {
                switch (foundHeader)
                {
                    case false when line.StartsWith(AutogeneratedCustomSectionHeaderPrefix, StringComparison.Ordinal):
                        foundHeader = true;
                        continue;
                    case false:
                        continue;
