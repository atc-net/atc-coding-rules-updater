...
namespace Atc.CodingRules.Updater.CLI
{
    public static class EditorConfigHelper
    {
        ...
        public static Task UpdateFileWithCustomAtcAutogeneratedRuleSuppressions(
            ...
            IList<Tuple<string, List<string>>> suppressionLinesPrAnalyzer)
        {
            ...
            var isFirstRun = !lines.Any(x => x.StartsWith(AutogeneratedCustomSectionHeaderPrefix, StringComparison.Ordinal));

            var contentToWrite = isFirstRun
                ? UpdateFileWithCustomAtcAutogeneratedRuleSuppressionsHandleFirstRun(suppressionLinesPrAnalyzer, lines)
                : UpdateFileWithCustomAtcAutogeneratedRuleSuppressionsHandleUpdate(analyzerProviderBaseRules, suppressionLinesPrAnalyzer, lines);

            return File.WriteAllTextAsync(rootEditorConfigFile.FullName, contentToWrite, Encoding.UTF8);
        }

        private static string UpdateFileWithCustomAtcAutogeneratedRuleSuppressionsHandleUpdate(
            ...
            string[] lines)
        {
            ...
            var existingLinesBeforeCustomAnalyzerSuppressions = ExtractBeforeCustomAutogeneratedRulesHeader(lines);
            var existingCustomAnalyzerSuppressions = GetExistingCustomAnalyzerSuppressions(lines, analyzerProviderBaseRules);

            foreach (var (analyzerName, suppressionLines) in suppressionLinesPrAnalyzer)
            {
                var existingCustomAnalyzer = existingCustomAnalyzerSuppressions.SingleOrDefault(x => x.Item1.Equals(analyzerName, StringComparison.Ordinal));
                if (existingCustomAnalyzer is not null)
                {
                    existingCustomAnalyzer.Item2.AddRange(suppressionLines);
                }
                else
                {
                    existingCustomAnalyzerSuppressions.Add(Tuple.Create(analyzerName, suppressionLines));
                }
            }

            foreach (var line in existingLinesBeforeCustomAnalyzerSuppressions)
            {
                sb.AppendLine(line);
            }

            sb.AppendLine(SectionDivider);
            sb.AppendLine($"{AutogeneratedCustomSectionHeaderPrefix} {DateTime.Now:F}");
            sb.AppendLine(SectionDivider);

            foreach (var (analyzerName, suppressionLines) in existingCustomAnalyzerSuppressions)
            {
                sb.AppendLine($"{Environment.NewLine}# {analyzerName}");

                foreach (var suppressionLine in suppressionLines.OrderBy(x => x))
                {
        ...
        private static string UpdateFileWithCustomAtcAutogeneratedRuleSuppressionsHandleFirstRun(IEnumerable<Tuple<string, List<string>>> suppressionLinesPrAnalyzer, IEnumerable<string> lines)
        {
            var sb = new StringBuilder();
        ...
        private static IList<Tuple<string, List<string>>> GetExistingCustomAnalyzerSuppressions(
            string[] lines,
        ...
        private static string[] ExtractBeforeCustomAutogeneratedRulesHeader(string[] lines)
        {
            if (lines == null)
            {
                throw new ArgumentNullException(nameof(lines));
